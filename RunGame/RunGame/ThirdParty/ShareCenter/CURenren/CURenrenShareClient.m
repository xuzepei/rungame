...
- (NSString *)userId
{
    if ([renren isSessionValid])
    {
        ...
        if (nil == uid)
        {
            ...
            NSTimeInterval expireTime = [[NSDate date] timeIntervalSince1970] + 20;
            
            NSRunLoop * runloop = [NSRunLoop currentRunLoop];
            while (nil == (uid = [defaults objectForKey:@"session_UserId"]) && [[NSDate date] timeIntervalSince1970] < expireTime)
            {
...
- (NSString *)accessToken
{
    if ([renren isSessionValid])
    {
        return [[renren.accessToken copy] autorelease];
    }
...
- (NSTimeInterval)expireTime
{
    if ([renren isSessionValid])
    {
        return [renren.expirationDate timeIntervalSince1970];
    }
...
{
    if(page < 1 || ![self isCUAuth])
        return;
    
    if(NO == [renren isSessionValid])
    {
        return;
    }
    
    ROGetFriendsRequestParam *param = [[ROGetFriendsRequestParam alloc] init];
    param.page = [NSString stringWithFormat:@"%d",page];
    param.count = @"20";
    
    [renren getFriends:param
               succeed:^(NSString * responseString){
                   DebugLog(@"++++++ GET RenRen Friends: %@", responseString);
