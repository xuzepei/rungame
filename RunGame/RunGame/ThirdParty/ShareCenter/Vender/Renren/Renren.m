...
- (void)requestWithParam:(RORequestParam *)param andDelegate:(id <RORequestDelegate>)delegate {
    ...
    if (![self isSessionValid]) {
        DebugLog(@"Session is not valid! Request abort!!");
        return;
    }
                 ...
                 failure:(void (^)(NSError *))failureBlock
{
    ...
    self.request = [RORequest getRequestWithParam:param httpMethod:@"POST" delegate:nil requestURL:kRestserverBaseURL];
    self.request.completeBlock = completeBlock;
    self.request.failureBlock = failureBlock;
                         ...
                         failure:(void (^)(NSError *))failureBlock
{
    ...
    RORequest *aRequest = [RORequest getRequestWithParams:params httpMethod:@"GET" delegate:nil requestURL:kRestserverBaseURL];
    aRequest.completeBlock = completeBlock;
    aRequest.failureBlock = failureBlock;
...
- (void)request:(RORequest *)request didFailWithError:(NSError*)error{
    DebugLog(@"Failed to expire the session");
}
 ...
 */
-(void)getFriends:(ROGetFriendsRequestParam *)param
          succeed:(void (^)(NSString *))completeBlock
          failure:(void (^)(NSError *))failureBlock
{
