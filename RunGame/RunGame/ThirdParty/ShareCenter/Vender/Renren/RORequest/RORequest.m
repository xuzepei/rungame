...
- (void)failWithError:(NSError *)error {
    ...
    else if([_delegate respondsToSelector:@selector(request:didFailWithROError:)])
    {
...
- (void)connect{
     ...
     */
	//DebugLog(@"Here's the request headers: %@", [urlRequest allHTTPHeaderFields]);
    //DebugLog(@"Here's the request body: %@", [urlRequest HTTPBody]);
...
- (void)handleResponseData:(NSData *)data{
    ...
    {
        NSError* error = nil;
        [self parseJsonResponse:data error:&error];
        ...
        {
            NSString* responseString = [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease];
            DebugLog(@"Here's the response string: %@", responseString);
            self.completeBlock(responseString);
        }
    }
    else if([_delegate respondsToSelector:@selector(request:didLoad:)] || [_delegate respondsToSelector:@selector(request:didFailWithError:)])
    {
        NSError* error = nil;  
