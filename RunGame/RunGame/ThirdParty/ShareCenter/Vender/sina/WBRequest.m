...
- (void)dealloc
{
    ...
    [_completeBlock release]; _completeBlock = nil;
    [_failureBlock release]; _completeBlock = nil;
...
- (id)parseJSONData:(NSData *)data error:(NSError **)error
{
	...
	id result = [dataString objectFromJSONStringWithParseOptions:0 error:&parseError];
...
- (void)failedWithError:(NSError *)error 
{
	...
	else if ([delegate respondsToSelector:@selector(request:didFailWithError:)])
	{
...
- (void)addPostImage:(UIImage *)image forKey:(NSString *)key
{
    [params setValue:image forKey:key];
}
- (void)addPostFileData:(NSData *)data forKey:(NSString *)key
{
